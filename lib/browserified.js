...
configDefault = {
	...
	loadSigSounds 		: true,						// 시그니쳐 사운드를 불러올지
	sigSoundsURI			: "",								/* 불러올 시그니쳐 사운드 Uri.
																					 로컬 시그니쳐 사운드을 이용할 경우 공백으로 둔다.		*/
...
var applySigSound = function(message, data){ return message; };
var applyMessage = function(message, data) {
	...
	message = applySigSound(message, data);
...
var checkComplete = function() {
	...
	if (++completeCount == num) {
		var chat = addChatMessage("",
			...
			"color:white; background:black; font-size:20px, font-family:'굴림체'; " +
...
sigSoundsData = [];
sigSound = new Audio();
if (configData.loadSigSounds) {
	var sigSoundsSubURI = "sounds/";
	var sigSoundsMainURI = "";
	if (configData.sigSoundsURI == "") {
		configData.sigSoundsURI = "./";
		sigSoundsSubURI = "sounds/";
		sigSoundsMainURI = "lib/";
	}
	else if (configData.sigSoundsURI[configData.sigSoundsURI.length-1] != "/") {
		configData.sigSoundsURI += "/";
	}

	var sigSoundScript = document.createElement("script");
	sigSoundScript.type = "text/javascript";
	sigSoundScript.charset = "utf-8";
	sigSoundScript.src = configData.sigSoundsURI + sigSoundsMainURI + "sigsound_list.js";
	document.body.appendChild(sigSoundScript);

	sigSoundScript.onload = function() {
		if (sigSoundsData.length == 0) { debugLog("시그니쳐 사운드를 불러오는 데 실패했습니다."); }
		else {
			var keywords = [];
			for (var index in sigSoundsData) {
				for (var index2 in sigSoundsData[index].keywords) {
					keywords.push(sigSoundsData[index].keywords[index2]);
				}
			}
			keywords.sort(function(a,b) { return b.length - a.length; } );

			applySigSound = function(message, data) {
				for (var index in keywords) {
					var keyword = keywords[index];
					if (message.indexOf("#" + keyword) != -1) {
						// message = keyword + message.split("#" + keyword).join(
						// 	'<audio autoplay src="' +
						// 	configData.sigSoundsURI + sigSoundsSubURI +
						// 	sigSoundsData.find( function(element) {
						// 		return element.keywords.indexOf(keyword) != -1;
						// 	} ).name + '" />');

						var sigSoundURI = configData.sigSoundsURI + sigSoundsSubURI +
							sigSoundsData.find( function(element) {
								return element.keywords.indexOf(keyword) != -1;
							} ).name;

						if (sigSound.paused) {
							sigSound = new Audio(sigSoundURI);
							sigSound.src = sigSoundURI;
							sigSound.volume = "0.3";
							sigSound.play();
							message = keyword;
						} else {
			...
			debugLog("시그니쳐 사운드를 적용했습니다.");
			checkComplete();
		}
	};
}
else {
	debugLog("설정에 따라 시그니쳐 사운드를 적용하지 않았습니다.");
	checkComplete();
}
